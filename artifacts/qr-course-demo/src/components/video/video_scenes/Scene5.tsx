...
export function Scene5({ setCursorPos, setIsClicking }: { setCursorPos: (pos: {x: string, y: string}) => void, setIsClicking: (val: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    const t3 = setTimeout(() => {
      setTypedAnswer("Because the words are spelled similarly.");
    }, 3000);
    ...
    const t9 = setTimeout(() => {
      ...
      setTypedAnswer("Because songs");
    }, 8500);
    const t10 = setTimeout(() => setTypedAnswer("Because songs that sit close together"), 9000);
    const t11 = setTimeout(() => setTypedAnswer("Because songs that sit close together share similar"), 9500);
    const t12 = setTimeout(() => setTypedAnswer("Because songs that sit close together share similar features, so liking one suggests the other."), 10000);
    ...
    >
      ...
      <div className="max-w-4xl mx-auto w-full">
        ...
        <div className="flex justify-between items-end mb-8">
          <div>
            <h1 className="text-4xl font-serif text-primary mb-2">Vectors and embeddings</h1>
        ...
        <div className="bg-white border border-border rounded-xl shadow-sm overflow-hidden flex flex-col min-h-[400px]">
          <div className="p-8 flex-1 border-b border-border text-lg text-primary font-medium leading-relaxed">
            {phase >= 1 && phase < 4 && (
              <StreamingText text="In an embedding, the words 'cat' and 'kitten' end up as points that sit very close together, while 'cat' and 'airplane' sit far apart. In plain words, what does that closeness tell us?" delay={0} />
            )}
            {phase >= 4 && (
              <StreamingText text="A music app places two songs you love as points that sit very close together in its 'taste space.' Why might that make it suggest one song right after the other?" delay={0} />
            )}
          </div>
          ...
          <div className="p-6 bg-muted/20">
              ...
              {phase >= 2 && phase < 4 && (
                ...
                >
                  ...
                  <div className="text-sm text-red-900 leading-relaxed">
                    <StreamingText text="Not quite. Points sitting close together means the words have similar meaning, not similar spelling. An embedding places things by what they mean, so 'cat' and 'kitten' land near each other because they're related in meaning." delay={0} />
                  </div>
                </motion.div>
              )}
              {phase >= 5 && (
                ...
                >
                  ...
                  <div className="text-sm text-emerald-900 leading-relaxed">
                    <StreamingText text="Correct! Points that sit close together represent things with similar features. So when two songs land near each other in taste space, liking one is a strong hint you'll enjoy the other — that's how similarity drives recommendations." delay={0} />
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <div className="mt-4">
              <div className="flex space-x-2 border-b border-border pb-2 mb-3 overflow-x-auto text-xs font-medium">
                <div className="px-3 py-1 border-b-2 border-primary text-primary">Vectors & embeddings</div>
                {['Dot product', 'Matrices', 'Gradients', 'Gradient descent', 'Probability', 'Backpropagation'].map(t => (
