...
router.get("/diagnostics/system", async (_req, res) => {
  ...
  steps.push(
    await run("Database: course content seeded", async () => {
      ...
      if (t.length < 8) throw new Error(`only ${t.length} topics`);
    ...
    steps.push(
      ...
      await run(`Practice problem ${i + 1}: generate + grade + adapt`, async () => {
        ...
        const gen = await chatJson<{
          ...
          `You generate a single introductory developmental psychology practice problem on "${topic.title}" at easy difficulty, with a short answer (a word, term, "yes"/"no", or short phrase). Respond as strict JSON: {"prompt": string, "correctAnswer": string, "explanation": string}.`,
    ...
    steps.push(
      ...
      await run(
        ...
        async () => {
          ...
          // developmental psychology. Homework is scoped to its own topic's lecture, but
          ...
          const derived = await chatText(
            "You are a strong introductory developmental psychology student answering a short-answer problem. " +
          ...
          const verdict = await chatJson<{
            ...
            "You are an academic quality-control reviewer for an introductory developmental psychology course. " +
...
router.post("/diagnostics/expand-lectures", async (req, res) => {
  ...
  const sys =
    `You are an introductory developmental psychology lecturer producing the ${level.toUpperCase()} version of a lecture. ` +
