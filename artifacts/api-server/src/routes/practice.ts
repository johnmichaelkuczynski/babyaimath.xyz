...
router.post("/practice/sessions/:sessionId/next", async (req, res): Promise<void> => {
  ...
  try {
    generated = await chatJson<{
      ...
      `You generate a single functional-intelligence (reasoning-test skills) practice problem for a curious beginner. The problem MUST be on the topic "${topic.title}" and at difficulty "${difficultyLabel}" (${difficulty.toFixed(
        1,
      )}/5). Test general understanding of the SUBJECT of reasoning-test methods, not recall of any particular book, lecture, or course. The question MUST be fully self-contained and answerable by anyone who knows the discipline: do NOT reference "the lecture", "the text", "the course", "the example", "the case", or any named character or example a student would only recognize from a specific reading.\n\nSTRICT QUESTION RULES (no exceptions):\n- Every question must present a SPECIFIC, concrete puzzle or test-taking scenario (e.g. a 3x3 grid where the number of dots rises by one across each row and down each column with one cell blank, the series 3 6 12 24 where each term doubles, the analogy 'thermometer is to temperature as clock is to ___', a spatial option that matches the target shape but looks subtly mirror-flipped, a timed no-penalty test where a solver is stuck three minutes on one item) and ask the student to APPLY the topic's idea to THAT scenario — to judge, compare, explain, interpret, or decide. Explain ideas intuitively, in plain words — keep any arithmetic light and reasoned, never a heavy formula crunch.\n- NEVER ask for a definition, a term, or what something is "called"; NEVER ask the student to recite an abstract formulation from any reading.\n- NEVER write a question whose answer is a single word, a single term, or a bare "yes"/"no". The answer must require reasoning that is HARD TO SHARE: the student must explain their thinking about the specific scenario in 2-4 sentences.\n- The "correctAnswer" is the model answer (2-4 sentences) showing the operational reasoning a strong student would give for THIS scenario. The "explanation" briefly says what earns full credit.\n\nRespond as strict JSON: {"prompt": string, "correctAnswer": string, "explanation": string}. Avoid these recent prompts: ${JSON.stringify(
    ...
    try {
      tutorTip = (
        await chatJson<{ tip: string }>(
          "You are a kind, concise functional-intelligence (reasoning-test skills) tutor. Given a problem, the correct answer, and the student's wrong attempt, give ONE focused next-step tip (2 sentences max). Respond as strict JSON: {\"tip\": string}.",
          JSON.stringify({
            prompt: problem.prompt,
            correctAnswer: problem.correctAnswer,
