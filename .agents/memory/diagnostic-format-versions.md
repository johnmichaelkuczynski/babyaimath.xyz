...
- **The Short/Medium/Long choice lives in the RUNNER as one prominent full-width picker, NOT as buttons on the list page.** The list page shows only the 3 format buttons (link to `/reasoning/:id`, no query); opening any diagnostic then shows the length picker before questions.
  **Why:** the user demanded ≥4× that "for each diagnostic there are short/medium/long options." An earlier attempt put 9 tiny per-format length buttons on the list that deep-linked with `?length=` and auto-started the runner — clicking one dropped the student straight into the test with no visible length step (exactly what they complained about). Routing format→runner→prominent picker makes the length choice unmissable for every diagnostic. Do NOT reintroduce `?length=` deep-link auto-start.
